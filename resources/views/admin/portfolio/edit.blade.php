...
@section('content')
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      <!-- Multi step form --> 
      <form action="{{route('admin.portfolio.update', $portfolio)}}" method="post" accept-charset="utf-8" class="cabinet-content__form">
        @csrf
        @method('PUT')
        <h3><b>Шаг 1.</b> Введите заголовок</h3>
        <div class="form-group">
          <label for="cardName"><b>Введите название</b></label>
          <input name="title" id="name" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}"  value="{{ $portfolio->title}}">
            @if ($errors->has('title'))
        ...
        <div class="form-group">
          ...
          <textarea name="body" id="editor" class="form-control{{ $errors->has('body') ? ' is-invalid' : '' }}">{{$portfolio->body}}</textarea>
            @if ($errors->has('body'))
              <span class="invalid-feedback">
